...
$podcasts = PHP::selectSiteAllDateCIN('tb_site.podcasts', ($paginaAtual - 1) * $porPagina, $porPagina);
?>

<div class="pt-5">
    <h3 class="py-5">Podcasts</h3>
</div>
...
<div class="container-fluid table-responsive">
    ...
    <div class="paginacao">
        <?php
        $totalPaginas = ceil(count(PHP::selectSiteAllCIN('tb_site.podcasts')) / $porPagina);

        for ($i = 1; $i <= $totalPaginas; $i++) {
            if ($i == $paginaAtual)
                echo '<a class="page-selected" href="' . INCLUDE_PATH . 'podcasts?pagina=' . $i . '">' . $i . '</a>';
            else
                echo '<a href="' . INCLUDE_PATH . 'podcasts?pagina=' . $i . '">' . $i . '</a>';
        }
