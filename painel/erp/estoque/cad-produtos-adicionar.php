...
<div class="box-content">
    <h3><i class="fa fa-plus"></i> Adicionar Produto</h3>

    <div class="right">
        <div><a href="<?php echo INCLUDE_PATH_PAINEL ?>cad-produtos-listar"><button class="btn btn-sm btn-success" type="button">Lista Produtos</button></a></div>
    </div>
    ...
    <form method="POST" enctype="multipart/form-data">
        ...
        if (isset($_POST['acao'])) {
            ...
            if ($descricao == '') {
                ...
                $verificar = MySql::conectar()->prepare("SELECT * FROM `tb_cad.produtos` WHERE CIN = ? AND descricao = ? AND EAN = ? ");
                $verificar->execute(array($_SESSION['CIN'], $_POST['descricao'], $_POST['EAN']));

                if ($verificar->rowCount() == 0) {

                    if (PHP::insertOne($_POST)) {
                        PHP::redirect(INCLUDE_PATH_PAINEL . 'cad-produtos-adicionar?sucesso');
                    }
                } else {
        ...
        <div class="row">
            <div class="col-sm-12 mt-3">
                <input type="hidden" name="nome_tabela" value="tb_cad.produtos" />
