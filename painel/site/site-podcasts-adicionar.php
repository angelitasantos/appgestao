...
<div class="box-content">
    <h3><i class="fa fa-plus"></i> Adicionar Podcasts</h3>
    ...
    <form method="POST" enctype="multipart/form-data">
        ...
        if (isset($_POST['acao'])) {
            ...
            if ($titulo == '' || $conteudo == '') {
                ...
                $verificar = MySql::conectar()->prepare("SELECT * FROM `tb_site.podcasts` WHERE CIN = ? AND titulo = ?");
                $verificar->execute(array($_SESSION['CIN'], $_POST['titulo']));
                if ($verificar->rowCount() == 0) {

                    $slug = PHP::generateSlug($titulo);
                    $array = ['CIN' => $_SESSION['CIN'], 'titulo' => $titulo, 'categoria' => $categoria, 'datapublicacao' => $datapublicacao, 'tags' => $tags, 'conteudo' => $conteudo, 'arquivo' => $arquivo, 'slug' => $slug, 'nome_tabela' => 'tb_site.downloads'];
                    if (PHP::insertOne($array)) {
                        PHP::redirect(INCLUDE_PATH_PAINEL . 'site-podcasts-adicionar?sucesso');
                    }
                } else {
